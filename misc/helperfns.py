''' whole file taken from https://github.com/BethanyL/DeepKoopman'''
...
def prepare_data(file_prefix, params):
    """
    loads and prepares data such that it can be used for training/validation.
    :param file_prefix:  prefix of files where data is. Data should be saved as
    file_prefix+<States/Inputs/Time>.csv
    :param params: dictionary that contains the parameters
    :return:
    """
    delay = 0
    if 'delay_coordinates_delay' in params:
        delay = params['delay_coordinates_delay']

    # load data and test if dimension of data is ok
    state_data_unstacked = pd.read_csv(Path(__file__).parents[1] / Path(file_prefix + 'States.csv')).to_numpy()
    # state_data_unstacked = np.loadtxt(Path(file_prefix + 'States.csv'))

    if 'system_outputs' in params:
        state_data_unstacked = state_data_unstacked[:, params['system_outputs']]

    state_height, state_width = state_data_unstacked.shape
    if state_width * (delay + 1) != params['dim_system_state']:
        print(params['dim_system_state'])
        raise NameError('File does not contain the same state number as specified in params[dim_system_state]')

    input_data_unstacked = pd.read_csv(Path(__file__).parents[1] / Path(file_prefix + 'Inputs.csv')).to_numpy()
    # input_data_unstacked = np.loadtxt(file_prefix + 'Inputs.csv')
    # convert array 1d array (whe input dimension is 1) into 2d array
    if input_data_unstacked.ndim == 1:
        input_data_unstacked = np.transpose(np.array([input_data_unstacked]))
    input_height, input_width = input_data_unstacked.shape
    if input_width != params['dim_system_input'] and not params['use_input_time_delay']:
        raise ValueError('File does not contain the same input number as specified in params[dim_system_ínput]')
    if input_width * (delay + 1) != params['dim_system_input'] and params['use_input_time_delay']:
        raise ValueError('File does not contain the same input number as specified in params[dim_system_ínput]')
    if state_height != input_height:
        raise ValueError('Inputs and states files dont have the same length')
    length_trajectories = params['prediction_horizon'] + 1
    if int(state_height / (length_trajectories + delay) * (length_trajectories + delay))\
            != state_height:
        raise ValueError('File does not contain an integer number of trajectories')

    num_trajectories = int(state_height / (length_trajectories + delay))

    # stack data (like in Lusch et al. just changed the dimensions)
    states = np.zeros((num_trajectories, length_trajectories, params['dim_system_state']),
                      dtype=float)
    inputs = np.zeros((num_trajectories, length_trajectories, params['dim_system_input']),
                      dtype=float)

    for i in range(0, num_trajectories):
        states[i, :, 0:state_width] = state_data_unstacked[
                          i * (length_trajectories+delay) + delay:(i + 1) * (length_trajectories+delay), :]
        for j in range(1, delay+1):
            states[i, :, state_width*j:state_width*(j+1)] = \
                state_data_unstacked[i * (length_trajectories+delay) + delay - j:
                                     (i + 1) * (length_trajectories+delay) - j, :]
        inputs[i, :, 0:input_width] = input_data_unstacked[
                          i * (length_trajectories+delay) + delay:(i + 1) * (length_trajectories+delay), :]
        if params['use_input_time_delay']:
            for j in range(1, delay + 1):
                inputs[i, :, input_width * j:input_width * (j + 1)] = \
                    input_data_unstacked[i * (length_trajectories + delay) + delay - j:
                                         (i + 1) * (length_trajectories + delay) - j, :]
...
def choose_optimizer(params, regularized_loss, trainable_var, name='Opt', learning_rate=1e-3):
    ...
    if params['opt_alg'] == 'adam':
        optimizer = tf.compat.v1.train.AdamOptimizer(
            learning_rate, name=name).minimize(regularized_loss, var_list=trainable_var)
        ...
        optimizer = tf.compat.v1.train.FtrlOptimizer(
            params['learning_rate']).minimize(regularized_loss, var_list=trainable_var)
        ...
        if params['decay_rate'] > 0:
            optimizer = tf.compat.v1.train.RMSPropOptimizer(
                params['learning_rate'], params['decay_rate']).minimize(regularized_loss, var_list=trainable_var)
        else:
...
def save_params(params, name):
    ...
    """
    pickle.dump(params, open(params['model_path'] + 'params' + name + '.pkl', 'wb'), pickle.HIGHEST_PROTOCOL)
